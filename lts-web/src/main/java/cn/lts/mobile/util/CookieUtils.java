...
public final class CookieUtils {
        ...
        for (int i = 0; i < cookieList.length; i++) {
            if (cookieList[i].getName().equals(cookieName)) {
                try {
                    ...
                    LOGGER.error("Cookie Decode Error1.", e);
                }catch (Exception e) {
                    LOGGER.error("Cookie Decode Error2.", e);
                }
            }
        }
        return retValue;
			...
			for (int i = 0; i < cookieList.length; i++) {
				if (cookieList[i].getName().equals(cookieName)) {
				    try {
					    ...
					    LOGGER.error("Cookie Decode Error3.", e);
					}catch (Exception e) {
                LOGGER.error("Cookie Decode Error4.", e);
            }
					break;
				}
		}
    ...
    /**
     * 获取域名根路径的方法： www.2caipiao.com 返回 2caipiao.com
     * @param serverName
     * @return
     */
    public static String getDomain(String serverName) {
        String domainRegex = "(?<= http://|\\.)([^.]*?\\.com|cn|net|org|biz|info|cc|tv)";
        String domain = "";
